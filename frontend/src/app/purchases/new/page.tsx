...
import { Plus, Minus, Save, ArrowLeft, ShoppingCart } from "lucide-react";
import { api } from "@/lib/api";

// Form validation schema
const purchaseFormSchema = z.object({
  vendorId: z.string().min(1, "Vendor is required"),
  paymentMethod: z.enum(["cash", "credit"], "Payment method is required"),
    ...
    const fetchData = async () => {
      try {
        ...
        if (vendorsResponse.success) {
          console.log(vendorsResponse.data.vendors);
  ...
  const onSubmit = async (data: PurchaseFormValues) => {
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || "Failed to save purchase");
      }
    } catch (err: unknown) {
      console.error("Error creating purchase:", err);
      setError((err as any).response?.data?.message || "Error saving purchase");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-primary">New Purchase</h1>
            <p className="text-muted-foreground">
              Save a new purchase transaction
            </p>
          </div>
        </div>
              ...
              <CardContent className="space-y-4">
                {fields.map((field, index) => {
                  ...
                  return (
                    <div
                      key={field.id}
                      className="grid gap-4 md:grid-cols-5 items-end p-4 border rounded-lg"
                    >
                      <FormField
                        control={form.control}
                ...
                {/* {fields.some((_, index) => {
                  ...
                  <Alert variant="destructive">
                    <AlertDescription>
                      Some products have insufficient stock. Please check the
                      quantities.
                    </AlertDescription>
                  </Alert>
                )} */}
