...
export default function AddSalePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (customersResponse.success) {
          console.log(customersResponse.data.customers);
  ...
  const onSubmit = async (data: SaleFormValues) => {
    try {
      ...
      setError((err as any).response?.data?.message || "Error saving sale");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-primary">New Sale</h1>
            <p className="text-muted-foreground">Save a new sale transaction</p>
          </div>
        </div>
